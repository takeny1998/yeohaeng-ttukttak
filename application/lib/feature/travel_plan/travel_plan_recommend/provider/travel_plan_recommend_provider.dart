...
class TravelPlanRecommend extends _$TravelPlanRecommend {
  ...
  TravelPlanRecommendState build(int travelId, CityModel city) {
    ...
    return TravelPlanRecommendState.fromTravel(city, travel);
  }
  ...
  void random() async {
    ...
    final queryParams = {'cityId': '${state.city.id}', 'category': category.name};

    final target = ListUtils.random(state.targets);
